...
import { Clock, ChevronLeft, ChevronRight, CheckCircle2, X, Lightbulb, Loader2, CheckSquare, Circle } from 'lucide-react';
  ...
  const handleSubmit = async (force = false) => {
    ...
    if (!force) {
      const result = await Swal.fire({
        title: 'Nộp bài thi?', text: "Sếp có chắc chắn muốn kết thúc bài thi?", icon: 'question', showCancelButton: true, confirmButtonColor: '#10b981', confirmButtonText: 'Đồng ý nộp!', cancelButtonText: 'Làm tiếp'
      });
    ...
    <div className="min-h-screen bg-[#f8f9fc] font-sans flex flex-col text-slate-900">
      ...
      {isSubmitted && score && (
        <div className="max-w-7xl mx-auto w-full px-6 lg:px-8 mt-8">
          <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-md flex flex-col md:flex-row items-center gap-8 bg-gradient-to-br from-white to-slate-50 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#4f46e5]/5 rounded-full -mr-16 -mt-16"></div>
            
            <div className="relative w-40 h-40 flex shrink-0 items-center justify-center rounded-full bg-slate-50 border-8 border-[#4f46e5]/10">
              ...
              <div className="text-center"><span className="text-4xl font-black text-[#1e1b4b]">{score.score}</span><span className="text-slate-400 font-bold"> / 10</span></div>
            </div>

            <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm text-center">
                <div className="text-slate-500 font-bold mb-1 text-sm uppercase">Số câu đúng</div>
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm text-center">
                <div className="text-slate-500 font-bold mb-1 text-sm uppercase">Thời gian làm</div>
                <div className="text-2xl font-black text-[#f59e0b]">{formatTime(score.time_spent)}</div>
              </div>
              <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm text-center">
                <div className="text-slate-500 font-bold mb-1 text-sm uppercase">Xếp loại</div>
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full flex flex-col lg:flex-row gap-8 p-6 lg:p-8">
              ...
              return (
                <button 
                  ...
                  className={`w-full text-left p-5 rounded-2xl border-2 transition-all flex items-center gap-5 group ${
                    isSelected ? 'border-[#4f46e5] bg-[#4f46e5]/5' : 'border-slate-100 hover:border-[#4f46e5]/40 hover:bg-slate-50'
                ...
                >
                  <div className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center font-bold text-lg transition-colors ${
                    ...
                    {/* Hiển thị Icon khác nhau cho Multiple / Single */}
                    {currentQ.question_type === 'multiple' 
                      ? (isSelected || isCorrect || isWrong ? <CheckSquare size={18} /> : <div className="w-4 h-4 border-2 border-current rounded-sm"></div>)
                      : opt
                    }
                  </div>
        ...
        <div className="w-full lg:w-80 flex flex-col gap-6 shrink-0 h-fit">
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="grid grid-cols-5 gap-3">
              {quiz.questions.map((q, idx) => {
                ...
                // Sửa logic Sidebar: Nếu Multiple mà mảng rỗng thì coi như chưa trả lời
                ...
                return (
                  <button
                    ...
                    className={`aspect-square rounded-xl font-bold flex items-center justify-center text-sm transition-all ${
                      isCurrent ? 'ring-4 ring-[#4f46e5]/30 scale-110 z-10' : ''
                    } ${isAnswered ? 'bg-[#10b981] text-white border-transparent' : 'bg-[#f8f9fc] text-slate-400 border border-slate-200'}`}
                  >
                    {idx + 1}
                  </button>
                )
          ...
          {!isSubmitted ? (
            <button onClick={() => handleSubmit(false)} className="w-full py-4 rounded-3xl bg-gradient-to-r from-[#10b981] to-emerald-500 text-white font-black text-xl flex items-center justify-center gap-2 shadow-lg hover:-translate-y-1">
              <CheckCircle2 size={24} /> NỘP BÀI THI
            </button>
          ) : (
            <button onClick={() => navigate('/dashboard')} className="w-full py-4 rounded-3xl bg-[#1e1b4b] text-white font-black text-xl flex flex-center justify-center shadow-lg hover:-translate-y-1">
              VỀ TRANG CHỦ
            </button>
          )}
        </div>
